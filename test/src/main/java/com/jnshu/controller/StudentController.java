...
//        return "hello";
    ...
    @RequestMapping(value = "/home/{currentPage}", method = RequestMethod.GET)
    public String page(@PathVariable("currentPage") int currentPage, Model model) throws Exception {
        //定义页的行数
        p.setPageSize(10);
        //得出总记录数
        p.setTotalUsers(studentService.findUsers().size());
        //打印总行数
        log.debug("总记录行数" + p.getTotalUsers());
...
//如果当前页小于总页数下一页等于当前页加一
    ...
    public String test(int page, Model model) throws Exception {
        p.setTotalUsers(studentService.findUsers().size());
        p.setTotalPages(p.getTotalUsers() % p.getPageSize() == 0 ? p.getTotalUsers() / p.getPageSize() : p.getTotalUsers() / p.getPageSize() + 1);
        List<Student> list = studentService.findUsersByPage((page - 1) * p.getPageSize(), p.getPageSize());
        //下一页
        if (page < p.getTotalPages()) {
//如果当前页小于总页数下一页等于当前页加一
            p.setNextPage(page + 1);
            //将当前页赋值给 实体类中的当前页继续用currentPage去比较
